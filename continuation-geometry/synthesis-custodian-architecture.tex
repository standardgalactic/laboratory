...
\textit{Biological example}: The cerebellum preserves motor primitives learned in one task and deploys them in another task. The low-dimensional temporal structure is preserved (within-task relational geometry survives: $\rho \approx 0.83$). But the contextual orientation changes. A decoder trained to read the structure in task A fails to read it in task B, not because the structure vanished, but because it was rotated ($54°$ average rotation in granule cells) \cite{garcia2026}.
...
Garcia-Garcia and colleagues trained mice on two tasks with different sensorimotor demands but the same underlying temporal structure: action, delay, reward. The premotor cortex generated low-dimensional temporal primitives that generalized strongly between tasks ($r_{\text{cortex}} = 0.83$) \cite{garcia2026}.
