...
The temptation, when building infrastructure for this problem, is to reach directly for a watermark or a provenance record and treat persistence of that record as evidence of continued validity. This is a category error. A watermark, a cryptographic commitment, or a provenance chain can establish that a transformed artifact traces back to an earlier one. None of these mechanisms can, by itself, establish that the claims the earlier artifact made are still the claims the transformed artifact makes. The two questions are independent, and conflating them allows a robust but semantically stale pointer to authenticate a claim that no longer holds. This is the identical failure mode Chapter~\ref{ch:ch12-verify-seams} examined for a specification's own name: a name can be perfectly well resolved, everyone agrees which artifact is meant, while remaining entirely unresolved on whether the artifact's claims survived whatever produced the version currently in hand.
