...
Formally, let $\Omega_{\mathrm{global}}$ denote the domain including all materially affected agents. A system that appears productive on a restricted domain $\Omega_{\mathrm{local}}$ may fail to be productive when evaluated on $\Omega_{\mathrm{global}}$:
\[
