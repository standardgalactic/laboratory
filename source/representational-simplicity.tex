...
The proposed architecture integrates three previously distinct frameworks, building on earlier interface and protocol designs for structuring many-to-one and small-group conversation \cite{anderson2009dialog,anderson2011thinwalls,anderson2022bubblecity,brown2005worldcafe}. The Attention Compiler supplies the institutional and conversational problem: many participants submit questions, objections, conjectures, evidence, corrections, and partially formed ideas toward a recipient whose attention budget is finite. CLIO supplies a theory of admissibility-relative projection: a submission may be compressed only through a quotient sufficient for the full class of routing, explanation, testing, escalation, audit, and appeal operations the institution authorizes. HYDRA supplies a theory of structured assembly: the projected submission, participant-state estimate, prerequisite graph, archive neighborhood, provenance record, recipient model, and ambient relevance fields must glue into a coherent routing state before any disposition is treated as warranted.
...
Let \(X\) be a measurable state space of submissions and let \(\mathcal C\) be a space of conversational contexts, equipped throughout with the usual apparatus of measure-theoretic probability \cite{billingsley1995,durrett2019}. A context
\[
c\in\mathcal C
\]
...
This is the natural generalization of classical statistical sufficiency, in which a statistic is sufficient for a family of distributions when it preserves every distinction relevant to inference about the underlying parameter \cite{fisher1922,neyman1935,halmos1949,lehmann1998}.
...
When \(X\) and \(M_c\) are random variables with a specified joint distribution induced by \(P_c\) and \(q_c\), one candidate complexity measure, using the standard information-theoretic apparatus of entropy and mutual information \cite{cover2006}, is
\[
\Comp_{\mathrm{MI}}(q_c)
=
I(X;q_c(X)).
...
For operation \(\alpha\), an unbiased Horvitz-Thompson estimator \cite{horvitz1952} of pairwise collapse energy can be formed from sampled pairs:
\[
...
\right),
\]
in the spirit of knowledge-tracing models that estimate latent mastery from a sequence of observed interactions \cite{corbett1994}.
...
o_{t+1}
\]
be the resulting observation. A controlled state-space model has transition kernel, in the manner of a partially observed Markov decision process solved by dynamic programming \cite{puterman1994,bertsekas2017},
\[
P
\left(
\theta_{t+1}
...
U_i(p,t)\in\{0,1\}
\]
represent whether the participant understands \(p\) sufficiently to state its content, consequences, and relevant alternatives, a notion of demonstrated understanding as distinct from mere assent that draws on prior computational treatments of comprehension and autonomous cognition \cite{degyurky2014,anderson2022um1}.
...
Y_v
\]
be performance on \(v\). The causal quantity of interest, expressed in the do-notation of structural causal models \cite{pearl2009,hernan2020}, is
\[
...
If \(\mathscr F\) is a sheaf, every pairwise-compatible family has a unique global gluing, in the classical sense developed for sheaf theory and topos theory \cite{godement1958,maclane1992,curry2014}. In an implemented conversational system, however, the assigned structure may be only a presheaf, the overlap maps may be incomplete, or the local compatibility judgments may themselves be uncertain.
...
A_r.
\]
Strict inequality is required for positive recurrent stability under ordinary nondegenerate queueing assumptions \cite{kleinrock1975}.
\end{theorem}
...
Escalation can be formulated as a selective prediction problem in the sense of statistical learning theory \cite{vapnik1998,geifman2017}. Let
\[
Y_{ir}\in\{0,1\}
\]
...
Suppose \(F_r\) is nonnegative, monotone, and submodular with \(F_r(\varnothing)=0\) \cite{nemhauser1978}. Let \(\mathcal{S}^{*}\) maximize \(F_r(\mathcal{S})\) over all sets satisfying \(|\mathcal{S}|\leq k\). Then the greedy set \(\mathcal{S}_k\) satisfies
\[
F_r(\mathcal{S}_k)
\geq
\left(
...
\sigma_g^2=p_g(1-p_g),
\]
and let \(c_g^{A}\) be the cost of auditing one residual in the stratum. If \(n_g\) cases are audited from stratum \(g\), the approximate variance of the stratified estimator for the suppressed-value total, following classical stratified-sampling theory \cite{cochran1977}, is
\[
\Var(\widehat{T}_V)
\approx
\sum_{g=1}^{G}
...
\]
be an outcome model for value among suppressed residuals, and let \(\pi(x)\) be the audit inclusion probability. A doubly robust estimator for the mean suppressed value, following the augmented inverse-probability-weighting construction developed for missing-data and causal-inference problems \cite{bang2005,robins1994}, is
\[
...
c_i^{\Audit},
\]
where \(G_i^{\mathrm{case}}\) is the value of correcting the individual case, \(G_i^{\mathrm{struct}}\) is the value of repairing the shared mechanism, and \(c_i^{\Audit}\) is the audit cost. This resembles active learning \cite{settles2009}, but the target is broader than predictive accuracy. The audit may revise the quotient space, the prerequisite topology, the coupling model, the recipient relation, or the meaning of an admissible residual.
...
This distinction separates semantic novelty from distributional novelty, echoing the standard division between statistical and neural-network-based approaches to novelty detection \cite{markou2003a,markou2003b}. Semantic novelty concerns what remains after a particular submission has been reconstructed and compared with the archive. Distributional novelty concerns a change in the arrival process, dependency structure, failure topology, or joint configuration of many submissions. A familiar claim can therefore be semantically nonnovel and distributionally significant. Conversely, an individually unusual claim can be semantically novel while having no detectable population-level recurrence.
...
Fix a node \(v\) in the prerequisite or answer graph. Suppose submissions associated with \(v\) follow a Poisson process under the reference model \cite{daley2003}:
\[
N_v(t+\Delta)-N_v(t)
\sim
...
More generally, an information-bottleneck objective, in the sense introduced for compressing a signal while preserving relevant information \cite{tishby1999}, is
\[
\min_{p(z\mid x)}
\left[
I(X;Z)-\beta I(Z;Y_r)
...
Let \(G\in\{0,1\}\) denote a participant group, let \(V\in\{0,1\}\) denote true value, and let \(E\in\{0,1\}\) denote escalation. Several fairness conditions are plausible, and it is known that not all of them can be satisfied simultaneously except in degenerate cases \cite{kleinberg2017fairness,chouldechova2017}.
...
K(\mathcal{E})
\]
be the description length of an explanation, in the sense of Kolmogorov complexity \cite{li2019}. Suppose an explanation is strongly faithful when it permits exact reconstruction of the decision for every admissible perturbation in neighborhood \(U(\sigma)\).
...
This is the exact JPEG-like case generalized to an arbitrary declared function family \cite{mitchell2002}. Outside finite exhaustively verifiable domains, establishing the premise is difficult. The theorem clarifies what must be proved rather than allowing the projection to certify itself.
